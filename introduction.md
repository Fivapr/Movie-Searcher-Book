...
Наша цель здесь — сделать обращение к апи максимально простым:
...
В `utils/api` мы создадим кастомный [инстанс аксиоса](https://github.com/axios/axios#custom-instance-defaults) для нашего проекта. Этот инстанс помогает собрать строку запроса по кусочкам. В объект опций мы положим baseURL — бывший `https://api.themoviedb.org/3/`, и api_key в качестве первого из params. Этот объект конкатенирует к основному пути строки в стиле `&api_key=blablabla` и остальные параметры, например query поиска `&query=la la land`.

Тк мы не будем использовать post запросы и другие опции второго аргумента аксиоса кроме парамсов, то заэкспортим функцию, которая все это будет делать за нас. На самом верхнем уровне внутри компонентов лучше всего иметь максимально декларативный синтаксис, спрятав все детали реализации внутри `utils/api.js` и заэкспортить в итоге максильмано чистый вариант. Поэтому сразу вызываем .get и деструктуризируем params.
...
Ради интереса еще можно вставить в этот файл небольшой [интерсептор](https://github.com/axios/axios#interceptors)-логгер аксиоса, чтобы примерно понять, что это за либа такая аксиос, какой у нее реквест и парамсы и прикинуть, чем еще можно обмазывать инстанс аксиоса.
